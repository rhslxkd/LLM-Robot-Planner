"""
run_neural_astar_gratio_sweep.py
같은 32x32 grid에 g_ratio 여러 값(0.1/0.5/0.9)으로 Neural A*를 돌려서
경로 다양성이 실제로 나오는지 확인.
"""
import os
import numpy as np
...
obs_32_arr = (np.array(obs_32) > 127).astype(np.float32)
...
# ---------------- 3. 같은 지도 위에 g_ratio별 경로 겹쳐서 시각화 ----------------
...
ax.legend()
ax.set_title("Neural A* paths across g_ratio values")

out_path = os.path.join(OUT_DIR, "gratio_sweep_overlay.png")
plt.tight_layout()
plt.savefig(out_path, dpi=150)
print(f"saved: {out_path}")

# 경로가 실제로 다른지 픽셀 집합 IoU로 수치 비교 (1.0=완전 동일)
