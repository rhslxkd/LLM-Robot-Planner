    ...
    try:
        ...
        SCENARIO_TEMPLATES = {
            ...
            "oracle_scene_E": """
                ...
                3. (5.5, 0.8)   - 동쪽으로 이동해서 첫 번째 가로 판의 오른쪽 끝 지점까지 이동
                4. (5.5, 1.7)   - 첫 번째 가로 판 위쪽으로 상승
                5. (2.0, 1.7)   - 서쪽으로 이동해서 두 번째 가로 판의 왼쪽 끝 지점까지 이동
                6. (2.0, 2.5)   - 두 번째 가로 판 위쪽으로 상승
                7. (7.0, 2.5)   - 최종 목표지점까지 동쪽으로 이동

                위 7개 체크포인트에 시작점 (0.0, 0.0)을 더해서 총 8개 지점을 지나야 하는데,
                이 8개 지점을 그대로 다 쓰거나, 그 사이사이에 자연스러운 중간 지점을 추가해서
                총 15개의 좌표로 만들어줘 (체크포인트 자체는 절대 순서를 바꾸거나 생략하면 안 돼).
                각 지점 사이는 부드러운 곡선으로 이어지게 하고, 웨이포인트 간 거리는 0.4m~1.0m를 유지해.
                이 체크포인트들은 이미 안전마진(0.5m)을 확보해서 계산된 좌표이니, 그대로 신뢰하고 사용해도 돼.
                """,
