...
def main():
    ...
    try:
        ...
        SCENARIO_TEMPLATES = {
            ...
            "oracle_scene_D": """
                중앙에 있는 로봇(go2)이 앞으로 가야하는 상황이야.
                이 통로 가운데를 통과해서 앞으로 7m 이동할 수 있는 {num_waypoints}개의 좌표를 제시해줘.
                통로를 가운데로 장애물을 통과해 지나가는 waypoints를 만들어줘. 
                통로 통과가 쉽지 않아보여도 시도할 가치가 있으니 용감하게 한번 통로 가운데를 지나가도록 설계해줘.(이게 최우선.)
                절대 우회하지마.
                """,
            "oracle_scene_E": """
                중앙에 있는 로봇(go2)이 앞으로 가야하는 상황이야.
