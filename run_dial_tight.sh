...
SCENES="R003 R004 R006 R009 R011"

run_and_kill() {
  local s=$1
  local scene="oracle_scene_${s}"
  local outdir="data/${scene}/no_vlm_tight"
  local before
  ...
  python3 -u dial_mpc/dial_mpc/core/dial_core.py \
    ...
    --output-dir "$outdir" \
    --n-steps 1000 \
    > "dial_batch_logs/${scene}_tight.log" 2>&1 &
  local pid=$!
  ...
  while [ "$waited" -lt 900 ]; do
    ...
    if ! kill -0 "$pid" 2>/dev/null; then
      echo "[$scene] ⚠️ states.npy 없이 프로세스 먼저 종료됨 (로그 확인: dial_batch_logs/${scene}_tight.log)"
      return 1
    fi
    sleep 5
    waited=$((waited + 5))
