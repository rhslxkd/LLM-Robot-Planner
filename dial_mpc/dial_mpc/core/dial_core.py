...
# GPU 환경에서 JAX의 XLA 컴파일러가 Triton GEMM을 사용하도록 설정하여 성능 향상
...
def main():
    ...
    #example폴더 내의 모든 .yaml 파일 목록을 자동으로 가져오기
    #get_example_path("") 는 보통 example 폴더 내의 파일 경로를 반환하는 함수
    example_dir = os.path.dirname(get_example_path("dummy"))
    yaml_files = glob.glob(os.path.join(example_dir, "*.yaml"))
    #파일명에서 확장자 제거 후 순수 예제 이름만 리스트화
    available_examples = [os.path.basename(f).replace(".yaml", "")for f in yaml_files]
    
    parser = argparse.ArgumentParser()
    config_or_example = parser.add_mutually_exclusive_group(required=False)
    config_or_example.add_argument(
        "--example",
        type=str,
        choices=available_examples, # 자동으로 갱신되는 리스트 주입!
        default=None, # 디폴트는 아래 로직에서 처리
    )
    ...
    if args.example is None and args.config is None:
        print(emoji.emojize(":light_bulb:") + " No arguments provided. Defaulting to 'unitree_go2_trot")
        args.example = "unitree_go2_trot"
    ...
    if args.example is not None:
        # get_example_path 함수가 examples/ 폴더에서 파일을 찾아줌
