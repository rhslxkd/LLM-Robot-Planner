...
@dataclass
class UnitreeGo2EnvConfig(BaseEnvConfig): # 이건 dataclass BaseEnvConfig를 상속받은 것(객체지향 프로그래밍)
    kp: Union[float, jax.Array] = 30.0 # 유니트리 Go2 로봇의 관절 위치 제어용 비례 이득
    kd: Union[float, jax.Array] = 0.0 # 유니트리 Go2 로봇의 관절 속도 제어용 미분 이득
    default_vx: float = 1.0 # 기본 전진 속도 설정
    default_vy: float = 0.0 # 기본 측면 속도 설정 측면속도가 뭐냐면 옆으로 가는 속도
...
class UnitreeGo2Env(BaseEnv):
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        pos = x.pos[self._torso_idx - 1] # torso 위치
        ...
        reward_pos = -jnp.sum((head_pos[:2] - pos_tar[:2]) ** 2)
        # stay upright reward
        ...
        ab = global_to_body_velocity( # 각속도 보상
            xd.ang[self._torso_idx - 1] * jnp.pi / 180.0, x.rot[self._torso_idx - 1]
        )
        reward_vel = -jnp.sum((vb[:2] - state.info["vel_tar"][:2]) ** 2) # 선속도 보상
        reward_ang_vel = -jnp.sum((ab[2] - state.info["ang_vel_tar"][2]) ** 2) # 각속도 보상
        # height reward
        reward_height = -jnp.sum( # 높이 보상
            (x.pos[self._torso_idx - 1, 2] - state.info["pos_tar"][2]) ** 2
        )
        # energy reward
        reward_energy = -jnp.sum( # 에너지 보상
        ...
        # reward
        reward = (
            reward_gaits * 0.1 # 보행 보상 
            + reward_air_time * 0.0 # 공중 체공 시간 보상
            ...
            + reward_height * 1.0 # 높이 보상
            + reward_energy * 0.00 # 에너지 보상
            + reward_alive * 0.0 # 살아있기 보상
        )
